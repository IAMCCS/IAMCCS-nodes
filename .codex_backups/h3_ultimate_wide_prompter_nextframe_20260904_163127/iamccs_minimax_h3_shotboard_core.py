...
def _longvid_guide_plan(
    *,
    ...
    for index, row in enumerate(visual_rows):
        ...
        authored_transition = _normalise_transition(row.get("transition"), len(visual_guides))
        guide = {
            ...
            "authored_hard_cut": authored_transition == "hard_cut",
        }
    ...
    # LONGVID and MULTI-SHOT share positioned-guide materialization, but they
    # do not share the same boundary semantics.  A real Shotboard hard cut is
    # an authored shot boundary: it must begin a fresh visual sample instead
    # of being placed in the tail of the preceding technical chunk and then
    # carried back through Motion Context.  Continuous guides keep the proven
    # R37 behavior unchanged.
    authored_hard_cut_frames = sorted(
        {
            int(guide["global_frame"])
            for guide in visual_guides
            if bool(guide.get("authored_hard_cut", False)) and int(guide["global_frame"]) > 0
        }
    )
    while cursor < requested_frames:
        remaining = requested_frames - cursor
        if motion_context_enabled:
            starts_authored_hard_cut = cursor in authored_hard_cut_frames
            chunk_visible_capacity = requested_motion_window if starts_authored_hard_cut else visible_capacity
            visible_frame_count = min(chunk_visible_capacity, remaining)
            next_authored_cut = next(
                (frame for frame in authored_hard_cut_frames if cursor < frame < cursor + visible_frame_count),
                None,
            )
            if next_authored_cut is not None:
                visible_frame_count = next_authored_cut - cursor
            trim_frames = 0 if starts_authored_hard_cut or not chunks else motion_context_tail_frames
            frame_count = align_h3_frames(visible_frame_count + trim_frames)
        else:
            starts_authored_hard_cut = False
        ...
        chunk = {
            ...
            **({
                ...
                "motion_context_reset_at_authored_cut": starts_authored_hard_cut,
            } if motion_context_enabled else {}),
            "fps": H3_FPS,
            "duration_seconds": frame_count / H3_FPS,
            **({"visible_duration_seconds": visible_frame_count / H3_FPS} if motion_context_enabled else {}),
            ...
            "transition": (
                (
                    "authored_hard_cut"
                    if starts_authored_hard_cut
                    else ("longvid_start" if cursor == 0 else "motion_context_continuation")
                )
                if motion_context_enabled
                else "longvid_hard_cut"
            ),
    ...
    return {
        ...
        **({
        ...
        "motion_context_auto_chain": {
            ...
            "reset_visual_context_at_authored_hard_cuts": True,
        },
        } if motion_context_enabled else {}),
        ...
        "chunk_policy": (
            f"longvid_motion_context_hard_cut_aware_visible_{visible_capacity}_window_{requested_motion_window}_model_max_{H3_MAX_TRAINED_FRAMES}"
            if motion_context_enabled
