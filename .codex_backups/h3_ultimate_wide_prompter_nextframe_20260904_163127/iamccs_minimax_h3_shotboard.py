    ...
    def plan(
        ...
        if fused_turbo_requested:
            if not fused_turbo_model or not _model_file_available("diffusion_models", fused_turbo_model):
                raise ValueError("Fused Turbo Preview needs a selected installed MiniMax H3 model in diffusion_models")
            if requested_turbo_mode != "off" or pdd_requested or secondary_lora_requested:
                raise ValueError("Fused Turbo Preview is a complete model profile: turn Turbo, PDD and secondary LoRA off")
            if str(task_mode or "auto_from_timeline").lower() not in {"t2va", "auto_from_timeline"}:
                raise ValueError("Fused Turbo Preview is intentionally isolated to T2VA. Use the standard H3 backend for I2V, FLF, REF2VA, V2V and LongVid.")
        ...
        if fused_turbo_requested:
            if any(str(chunk.get("task_mode", "")).lower() != "t2va" for chunk in plan.get("chunks", [])):
                raise ValueError("Fused Turbo Preview only supports pure T2VA chunks. The timeline selected an image/reference/guided task; use the standard H3 backend for that mode.")
...
class IAMCCS_ShotboardH3Settings:
    ...
    def INPUT_TYPES(cls):
        ...
        fused_labels = {
            "fused_turbo_model_name": (
                "15 · FUSED TURBO PREVIEW · complete diffusion model",
                "Select the installed MATLOWAI/Kijai fused MiniMax H3 INT8 ConvRot model from diffusion_models. This is not a LoRA and only supports pure T2VA.",
            ),
            "fused_turbo_sigma_preset": (
                "15 · FUSED TURBO PREVIEW · manual sigma profile",
