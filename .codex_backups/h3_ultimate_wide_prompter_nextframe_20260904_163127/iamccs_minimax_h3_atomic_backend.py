...
def _load_fused_turbo_preview_model(shotplan: dict[str, Any]):
    ...
    if not name or not folder_paths.get_full_path("diffusion_models", name):
        raise ValueError("Fused Turbo Preview model is unavailable in diffusion_models")
    try:
        ...
        raise RuntimeError(
            "Fused Turbo Preview requires SageAttention/Triton with a visible C compiler. "
    ...
    return model, f"Fused Turbo Preview | Kijai loader | {name} | Sage auto + fp16 accumulation"
...
def _accelerate(model, shotplan: dict[str, Any]):
    ...
    if mode == "matlowai_fused_turbo_manual_sigma":
        ...
        return model, "Fused Turbo Preview | Kijai Sage auto (isolated; no LoRA/cache stacking)"
...
class IAMCCS_MiniMaxH3AtomicModelRouter:
    ...
    def select(self, cine_linx, segment_index, fl2va_model=None, ref2va_model=None):
        ...
        if _is_fused_turbo_preview(shotplan):
            ...
            if str(task).lower() != "t2va":
                raise ValueError("Fused Turbo Preview router only supports a pure T2VA chunk")
            model, report = _load_fused_turbo_preview_model(shotplan)
            return model, "fused_turbo_t2va", report
        chunk = _chunk(cine_linx, segment_index)
        task = _effective_task(cine_linx, chunk)
        selected = self._input_name(cine_linx, segment_index)
    ...
    def render(
        ...
        if fused_turbo_active:
            if str(sampler_name).lower() != "euler" or float(denoise) != 1.0:
                raise ValueError("Fused Turbo Preview requires the visible profile values: Euler and denoise 1.0")
            if abs(float(shift_video) - 12.0) > 1e-6 or abs(float(shift_audio) - 3.0) > 1e-6:
                raise ValueError("Fused Turbo Preview requires the visible profile shifts: video 12.0 and audio 3.0")
        ...
        if fused_turbo_active:
            ...
            if preset not in sigma_map:
                raise ValueError(f"Unknown Fused Turbo Preview sigma preset: {preset}")
