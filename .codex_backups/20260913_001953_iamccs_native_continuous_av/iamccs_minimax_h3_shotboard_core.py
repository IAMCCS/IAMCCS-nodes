...
def _masked_loop_guided_plan(base: dict[str, Any], *, duration_seconds: float,
    ...
    if len(image_guides) < 2:
        raise ValueError(
            "MASKED LOOP GUIDED requires at least two chronological Shotboard image guides."
        )
    ...
    if indices[0] < 0 or any(b <= a for a, b in zip(indices, indices[1:])):
        raise ValueError(
            "MASKED LOOP GUIDED image guides must have unique, strictly increasing timeline positions."
        )
    ...
    if total_frames > 3600:
        raise ValueError(
            f"MASKED LOOP GUIDED needs {total_frames} frames, above the current 3600-frame looping ceiling. "
            "Shorten the timeline or split it into editorial reels."
        )
    for item in image_guides:
        if int(item.get("global_frame", 0)) >= total_frames:
            raise ValueError(
                f"MASKED LOOP GUIDED guide '{item.get('label') or item.get('id')}' lies outside the "
    ...
    if overlap not in {22, 39, 56}:
        overlap = 22
    if overlap >= window:
        raise ValueError("MASKED LOOP GUIDED overlap must be smaller than its technical window.")
    ...
    chunk = {
        "index": 0,
        "slot_index": 0,
        "slot_id": "masked_loop_guided_master",
        "slot_label": "Masked Loop Guided master",
        "task_mode": "t2va",
    ...
    base.update({
        ...
        "backend_revision": "masked-loop-guided-v2-window-prompts",
        "backend_variant": "iamccs_masked_loop_guided_v2",
        "task_mode": "longvid_masked_loop_guided",
        "generation_mode": "longvid_masked_loop_guided",
        "requested_task_mode": "longvid_masked_loop_guided",
        ...
        "mode_contract": {
            "mode": "longvid_masked_loop_guided",
    ...
    base["guide_track"] = {
        **guide_track,
        "mode": "longvid_masked_loop_guided",
        "backend": "iamccs_masked_loop_guided_v2",
        "events": events,
    }
    return base
...
def build_shotplan(
    *,
    ...
    # Compatibility: the former in-place masked-loop experiment is now routed
    # to the proven Herrgotts v1.2 direct AV-latent continuation contract.  The
    # serialized mode id is retained so existing boards open, but no masked
    # master-latent sampler is compiled anymore.
    ...
    } or herrgotts_requested
    legacy_motion_context_requested = continuous_guided_requested and not herrgotts_requested
    if continuous_guided_requested and not herrgotts_requested:
    ...
    for slot_index, slot in enumerate(slots):
        ...
        chunk = {
            ...
            **({
            ...
            "join_mode": (
                "herrgotts_phase_aligned_av"
                if herrgotts_requested and slot_index > 0
            ...
            "flf_anchor_contract": (
                "herrgotts_previous_full_av_latent_plus_authored_destination"
                if herrgotts_requested and slot_index > 0
                else (
            ...
            "frame_source": (
                "herrgotts_direct_av_flf_interval"
    ...
    return {
        ...
        "schema_version": 10,
        **({
            "backend_revision": "herrgotts-direct-av-v1.2.1",
            "backend_variant": "herrgotts_direct_av_chain_v1",
            "herrgotts_direct_av_chain": {
                "enabled": True,
                "provider": "Herrgotts-H3-Infinite-Continuation-Suite",
                "minimum_provider_version": "1.2.1",
                ...
                "preserve_authored_intermediate_keyframes": True,
                "manual_landing_tail_frames": 0,
                "semantics": "n_shotboard_guides_n_minus_one_flf_intervals",
            },
            "mode_contract": {
                "mode": "longvid_masked_loop_guided",
                "public_name": "Herrgotts Direct AV Continuation",
                "backend": "isolated_herrgotts_release_nodes",
                "former_masked_loop_removed": True,
                "legacy_backend_untouched": True,
                "requires": "Herrgotts-H3-Infinite-Continuation-Suite>=1.2.1",
            },
        } if herrgotts_requested else ({
