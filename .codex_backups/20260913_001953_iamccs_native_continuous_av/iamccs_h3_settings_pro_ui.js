...
const MODE_CHOICES = [
  ...
  ["HERRGOTTS · DIRECT AV", "longvid_masked_loop_guided", "FL2VA continuation through the provider's full AV-latent handover, automatic freeze-tail analysis and phase-aligned safe seam."],
...
function setAssistantMode(node, key) {
  ...
  if (key === "fl2va_stable") setValue(board || node, "flf_continuity_mode", "stable_keyframes", false);
  if (key === "fl2va_continuous") setValue(board || node, "flf_continuity_mode", "native_av_context", false);
  if (["ref2vid_lipsync", "longvid_guided_lipsync"].includes(key)) setShotboardAudio(node, "h3_custom_audio_drive");
  // Herrgotts v1.2 transports H3's complete generated AV latent. Switching
  // from LipSync must clear that mode's locked AudioBoard route so the active
  // mode and Queue Truth cannot disagree.
  if (key === "longvid_masked_loop_guided") setShotboardAudio(node, "h3_native_generated");
...
function isFastH3LoRA(name) { const value = String(name).toLowerCase(); return (value.includes("fasth3") || value.includes("fast_h3") || value.includes("fast-h3")) && !value.includes("fused"); }
...
function applyRecipe(node, recipe) {
  ...
  if (recipe === "native") {
    ...
    const asset = firstChoice(node, "turbo_lora_name", isFastH3LoRA); if (!asset) return;
...
function mount(node) {
  ...
  function visibleGroups(mode) {
    return GROUPS.filter((group) => group.assistant || group.dynamic || group.fields.some((name) => widget(node, name)));
  }
  ...
  function makeControl(name) {
    ...
    const hint = document.createElement("small"); hint.textContent = gate.available
      ? String(item.options?.tooltip || "Saved here and compiled at Queue.").replace(/RTX\s*30\d0/gi, "the selected VRAM tier")
      : `MUTED · ${gate.reason}`;
  ...
  function renderAssistant(mode) {
    ...
    addRecipe(speed, "FASTH3 · 6 STEP", "fasth3", Boolean(firstChoice(node, "turbo_lora_name", isFastH3LoRA)), selectedSpeed === "fasth3"); addRecipe(speed, "SLA · 4 STEP", "sla", Boolean(firstChoice(node, "turbo_lora_name", (name) => name.includes("sla") && name.includes(modeFamily(mode)))), selectedSpeed === "sla");
  ...
  function renderMain(mode) {
    ...
    recipeSet.forEach(([label, id]) => addRecipe(recipes, label, id, id !== "upres" || branchGate(node, "h3_upres_model_name").available));
    if (active === "overview") {
