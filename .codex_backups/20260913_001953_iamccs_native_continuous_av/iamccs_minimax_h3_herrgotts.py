...
"""IAMCCS Shotboard adapter for Herrgotts H3 Infinite v1.2.

This module deliberately does not reproduce the provider's continuation math.
It delegates start, direct AV-latent continuation, freeze-tail analysis and
context-aligned joining to the installed Herrgotts release nodes.  IAMCCS owns
only the Shotboard-to-segment translation and its existing sampling profiles.
"""
...
LOG = logging.getLogger("IAMCCS.MiniMaxH3.Herrgotts")
CATEGORY = "IAMCCS/MiniMax H3/Herrgotts Direct AV"
PROVIDER = "Herrgotts-H3-Infinite-Continuation-Suite"
...
def _provider_node(name: str):
    ...
    if cls is None:
        raise RuntimeError(
            f"IAMCCS Herrgotts Direct AV requires {PROVIDER} v1.2.1 or newer; "
            f"provider node '{name}' is unavailable. Enable/update the provider and restart ComfyUI."
        )
    return cls


def _ensure_provider_runtime(provider_cls):
    """Install Herrgotts' hooks before the first expensive H3 sample.

    Herrgotts deliberately waits until Continue.build() to install its global
    hooks.  An IAMCCS chain must preflight them before rendering clip one,
    otherwise a conflicting node pack wastes a complete sample before the
    error becomes visible.
    """
    module = importlib.import_module(provider_cls.__module__)
    package = getattr(module, "__package__", "")
    if not package:
        raise RuntimeError("Herrgotts provider package could not be resolved for runtime preflight")
    runtime = importlib.import_module(f"{package}.runtime_patches")
    ensure = getattr(runtime, "ensure_h3_runtime_patches", None)
    if not callable(ensure):
        raise RuntimeError("Herrgotts provider has no compatible runtime preflight API")
    try:
        ensure()
...
def _chunks(plan: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    if not isinstance(chunks, list) or not chunks:
        raise ValueError("Herrgotts Direct AV received an empty Shotboard interval plan")
...
def _contract(plan: dict[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(contract, dict) or not bool(contract.get("enabled")):
        raise ValueError(
            "Select HERRGOTTS DIRECT AV (the former Masked Loop button) in IAMCCS Settings/Shotboard."
        )
    return contract
...
class IAMCCS_MiniMaxH3HerrgottsSegment:
    """Inspect one dynamic Shotboard A→B interval used by the provider chain."""
    ...
    def inspect(self, cine_linx, segment_index):
        ...
        if index < 0 or index >= len(chunks):
            raise IndexError(f"Herrgotts segment_index={index} outside 0..{len(chunks) - 1}")
        chunk = chunks[index]
        ...
        if index == 0 and not torch.is_tensor(first):
            raise ValueError("Herrgotts interval 1 requires the first chronological Shotboard image")
        if not torch.is_tensor(last):
            raise ValueError(f"Herrgotts interval {index + 1} requires its destination Shotboard image")
        ...
        report = (
            f"IAMCCS → Herrgotts interval {index + 1}/{len(chunks)} | "
...
class IAMCCS_MiniMaxH3HerrgottsDirectAVChain:
    """Dynamic N-guide FL2VA chain delegated to Herrgotts v1.2 release nodes."""
        ...
        return {
            ...
            "optional": {
                "reference_image": ("IMAGE", {
                    "tooltip": "Optional Qwen-only <Picture 1> identity/style reference, exactly as in Herrgotts v1.2."
                }),
            },
        }
    ...
    def render(self, model, clip, video_vae, audio_vae, cine_linx, reference_image=None):
        ...
        if str(plan.get("audio_mode", "h3_native_generated")) not in {
            ...
            raise ValueError(
                "Herrgotts Direct AV v1.2 continues H3's native generated AV latent. "
        ...
        if context_frames not in {"5", "22", "39"}:
            raise ValueError("Herrgotts context must be one of 5, 22 or 39 frames")
        ...
        if preserve_authored_keyframes:
            LOG.info(
                "IAMCCS Herrgotts authored-keyframe priority | intermediate tails=preserved | "
        ...
        for index, chunk in enumerate(chunks):
            ...
            last = _load_image(last_path) if last_path else None
            if index == 0 and not torch.is_tensor(first):
                raise ValueError("Herrgotts clip 1 is missing the opening Shotboard guide")
            if not torch.is_tensor(last):
                raise ValueError(f"Herrgotts clip {index + 1} is missing its final Shotboard guide")
            requested_frames = max(5, int(chunk.get("requested_frame_count", chunk.get("frame_count", 5))))
            duration = requested_frames / float(H3_FPS)
            ...
            if index == 0:
                ...
                combined_images, combined_audio, trim_info = Join().join(
                    ...
                    video_crossfade_frames=int(contract.get("video_crossfade_frames", 4)),
            ...
            LOG.info(
                "IAMCCS Herrgotts Direct AV | clip=%d/%d | head=%df | output=%df",
        ...
        if not torch.is_tensor(combined_images) or int(combined_images.shape[0]) < 1:
            raise RuntimeError("Herrgotts Direct AV produced no final frames")
        bridge = combined_images[-1:].detach().clone()
        report = (
            "IAMCCS Herrgotts Direct AV chain complete | provider=Herrgotts v1.2 release nodes | "
...
NODE_CLASS_MAPPINGS = {
    "IAMCCS_MiniMaxH3HerrgottsSegment": IAMCCS_MiniMaxH3HerrgottsSegment,
    "IAMCCS_MiniMaxH3HerrgottsDirectAVChain": IAMCCS_MiniMaxH3HerrgottsDirectAVChain,
}

NODE_DISPLAY_NAME_MAPPINGS = {
    "IAMCCS_MiniMaxH3HerrgottsSegment": "MiniMax H3 · Herrgotts Shotboard Interval",
    "IAMCCS_MiniMaxH3HerrgottsDirectAVChain": "MiniMax H3 · Herrgotts Direct AV Chain",
}
