...
const HELP = {
    ...
    multi_shot_lipsync: ["Put one opening guide per intended shot in the main timeline; keep each speaking face readable.", "Place any continuous performance in AudioBoard and align clips with the slot magnets.", "GLOBAL fixes identities and world; LOCAL names the active speaker and silent listener for each shot.", "Select Long Multi-Shot. Technical H3 chunks can exceed the image count; their audio positions remain on the same clock.", "Review and inject prompts. Motion Context does not guarantee a visual morph between different images."],
...
export function createModeHelper(mode) {
    ...
    for (const [key,label] of Object.entries({t2va:"T2VA",i2va:"I2VA",fl2va:"FL2VA",ref2va:"REF2VA / LipSync",longvid_guides:"LongVid positioned guides",multi_shot_lipsync:"Long Multi-Shot",audio_driven:"Audio Driven",v2va_object_swap:"V2VA"})) helpSelect.append(new Option(label,key));
    ...
    note.textContent = "AI reference pictures can be read as one visual story and injected into Shotboard with H3 prompts. Connection alone never changes Queue: only visible Shotboard fields are final truth.";
