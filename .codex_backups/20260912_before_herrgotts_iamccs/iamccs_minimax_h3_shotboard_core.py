...
def build_shotplan(
    *,
    ...
    masked_loop_requested = requested_task_mode in {
        "longvid_masked_loop_guided", "masked_loop_guided", "long_masked_loop_guided",
    }
    if masked_loop_requested:
        # Reuse the proven Shotboard timeline parser, but collapse its global
        # guide track into one latent.  No existing LongVid/Motion Context plan
        # is altered by this opt-in branch.
        ...
        return _masked_loop_guided_plan(
            ...
            duration_seconds=max(
                max(0.01, _float(duration_seconds, 10.0)),
                _timeline_duration_seconds(timeline, duration_seconds),
            ),
    ...
    for slot_index, slot in enumerate(slots):
        ...
        chunk = {
            ...
            **({
            ...
            "join_mode": "motion_context_native_av" if motion_trim_frames else (resolved_join_mode if overlap else "none"),
            ...
            "flf_anchor_contract": (
                ...
                if continuous_guided_requested and motion_trim_frames
                else (
            ...
            "frame_source": "continuous_flf_interval" if continuous_guided_requested else "timeline_segment_trim",
        }
    ...
    return {
        ...
        **({
        ...
        "continuation_mode": (
            "direct_native_av_latent_plus_next_shotboard_destination"
            if continuous_guided_requested
            else (
        ...
        "upscale_mode": active_upscale_mode,
        "chunk_policy": (
            "n_guides_n_minus_one_continuous_flf_intervals"
            if continuous_guided_requested
            else (
