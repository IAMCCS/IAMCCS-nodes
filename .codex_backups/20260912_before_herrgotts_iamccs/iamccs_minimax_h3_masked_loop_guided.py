...
def _guide_batch(shotplan):
    ...
    if len(shapes) != 1:
        raise RuntimeError(
            "Masked Loop Guided could not normalize all keyframes to one master canvas: "
            + ", ".join(str(shape) for shape in sorted(shapes))
        )
    LOG.info("IAMCCS Masked Loop Guided keyframe canvas | %s", " | ".join(resize_notes))
...
def _master_chunk(shotplan):
    ...
    if not isinstance(chunks, list) or len(chunks) != 1 or not isinstance(chunks[0], dict):
        raise ValueError("Masked Loop Guided requires exactly one validated master chunk.")
    return chunks[0]
...
def _with_window_condition_envelope(conditioning, window_conditions):
    if not isinstance(conditioning, (list, tuple)) or not conditioning:
        raise RuntimeError("Masked Loop Guided conditioner received an invalid CONDITIONING object.")
    wrapped = []
...
class IAMCCS_MiniMaxH3MaskedLoopConditioning(IAMCCS_MiniMaxH3AtomicConditioningBackend):
    ...
    def prepare(self, *args, **kwargs):
        ...
        if cine_linx is None or clip is None:
            raise ValueError("Masked Loop Guided conditioning requires clip and cine_linx inputs.")
        ...
        if str(shotplan.get("task_mode", "")).strip().lower() != "longvid_masked_loop_guided":
            raise ValueError("This isolated conditioner only accepts MASKED LOOP GUIDED mode.")
        prompt_plan = _window_prompt_plan(shotplan)
        if not prompt_plan:
            raise RuntimeError("Masked Loop Guided produced no technical-window prompt plan.")
        ...
        LOG.info(
            "IAMCCS Masked Loop Guided conditioning | %s",
            " | ".join(
...
class IAMCCS_MiniMaxH3MaskedLoopGuidedSampler:
    ...
    def render(self, model, positive, latent, video_vae, audio_vae, cine_linx,
        ...
        if expected_windows > 1 and (
            ...
            raise RuntimeError(
                "Masked Loop Guided has multiple technical windows but received one shared prompt. "
...
NODE_DISPLAY_NAME_MAPPINGS = {
    "IAMCCS_MiniMaxH3MaskedLoopConditioning": "MiniMax H3 Masked Loop · Window Conditioning",
