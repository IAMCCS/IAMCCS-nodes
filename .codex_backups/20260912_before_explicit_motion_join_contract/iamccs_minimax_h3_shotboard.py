...
class IAMCCS_MiniMaxH3NativeCheckpointSave:
    ...
    def checkpoint(
        ...
        # Motion Context generation has already removed the injected AV head
        # before it reaches this checkpoint.  ``trim_head_frames`` remains on
        # the wire as an audit value, but it must not be reinterpreted as a
        # decoded-frame overlap request.  Doing so crossfades the previous
        # visible tail with the already unique head of the next chunk for
        # ~22 frames, producing the characteristic pause/dissolve seam.
        motion_context_pretrimmed = False
        if trim_count > 0 and isinstance(cine_linx, dict):
            try:
                checkpoint_plan = _resolve_shotplan(cine_linx)
                checkpoint_chunks = checkpoint_plan.get("chunks") if isinstance(checkpoint_plan, dict) else None
                checkpoint_chunk = (
                    checkpoint_chunks[current_segment]
                    if isinstance(checkpoint_chunks, list) and current_segment < len(checkpoint_chunks)
                    else {}
                )
                motion_contract = checkpoint_plan.get("motion_context_auto_chain") or {}
                motion_context_pretrimmed = bool(
                    isinstance(motion_contract, dict)
                    and motion_contract.get("enabled")
                    and int(checkpoint_chunk.get("motion_context_trim_frames", 0) or 0) == trim_count
                )
            except Exception as exc:
                LOG.warning("MiniMax H3 checkpoint could not inspect Motion Context join contract: %s", exc)
        overlap_stitch = trim_count > 1 and not motion_context_pretrimmed
